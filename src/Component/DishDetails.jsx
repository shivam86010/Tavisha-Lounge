...
import {
  ...
  Share2, Bookmark, ChevronLeft, ChevronRight, Sparkles,
  AlertCircle, CheckCircle, Utensils, Wine, Calendar,
  X, Info, MessageCircle, ShoppingBag, MapPin,
  Phone, Mail, Globe, ThumbsUp, Coffee, Sun,
  Moon, Sunrise, Sunset, Thermometer, Leaf, Flame,
  Camera
} from 'lucide-react';
...
const DISHES_DETAILS = {
  'MAHARAJA\'S THALI': {
    ...
    description: "Experience the grandeur of royal Indian dining with our signature Maharaja's Thali. This elaborate 24-course feast brings together the finest dishes from across India's royal kitchens, each prepared using centuries-old recipes.",
    longDescription: "The Maharaja's Thali is not just a meal; it's a journey through India's rich culinary heritage. Each course is carefully curated to represent different regions and royal houses, from the rich gravies of Lucknow to the fiery curries of Rajasthan.",
    highlights: [
      "Gold leaf garnish on select dishes",
      "Heirloom recipes from 7 royal houses",
      "12 regional specialties",
      "Traditional silver thali service",
      "Personalized chef's explanation"
    ],
    ingredients: [
    ...
    courseStructure: [
      {
        course: "Welcome Drinks",
        items: ["Chilled Rose Sarbat", "Masala Chaas", "Saffron Milk"],
        description: "Traditional welcome beverages"
      },
      {
        course: "Appetizers",
        items: ["Galouti Kebabs", "Paneer Tikka", "Fish Amritsari"],
        description: "Melt-in-your-mouth starters"
      },
      {
        course: "Main Course",
        items: ["Rogan Josh", "Butter Chicken", "Dal Makhani"],
        description: "Royal gravies and curries"
      }
    ],
    winePairing: [
      {
...
const GlassCard = ({ children, className = '' }) => (
  <div className={`bg-white/5 backdrop-blur-lg rounded-2xl border border-white/10 shadow-xl ${className}`}>
    {children}
  </div>
);

const GradientText = ({ children, className = '' }) => (
  <span className={`bg-gradient-to-r from-amber-400 via-orange-400 to-rose-400 bg-clip-text text-transparent ${className}`}>
    {children}
  </span>
);
...
const IconButton = ({ icon: Icon, onClick, className = '', children }) => (
  <button
    onClick={onClick}
    className={`group relative overflow-hidden rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-amber-500/50 transition-all duration-300 ${className}`}
  >
    ...
    <div className="absolute inset-0 -translate-x-full group-hover:translate-x-0 bg-gradient-to-r from-amber-500/20 to-transparent transition-transform duration-700" />
  </button>
);
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {/* Image Counter */}
        <div className="absolute top-6 right-6 bg-gray-900/80 backdrop-blur-sm px-4 py-2 rounded-full border border-amber-500/30">
          <span className="text-amber-400">{selectedImage + 1} / {images.length}</span>
        </div>
        
        {/* Zoom Indicator */}
        <div className="absolute bottom-6 left-6 bg-gray-900/80 backdrop-blur-sm px-4 py-2 rounded-xl border border-amber-500/30 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      {/* Lightbox */}
      {showLightbox && (
        <div className="fixed inset-0 z-50 bg-gray-900/95 backdrop-blur-xl flex items-center justify-center p-8">
          <button
            onClick={() => setShowLightbox(false)}
            className="absolute top-8 right-8 w-12 h-12 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20 transition-colors z-10"
          >
          ...
          <button
            onClick={() => setSelectedImage((prev) => (prev > 0 ? prev - 1 : images.length - 1))}
            className="absolute left-8 w-12 h-12 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20 transition-colors z-10"
          >
          ...
          <button
            onClick={() => setSelectedImage((prev) => (prev < images.length - 1 ? prev + 1 : 0))}
            className="absolute right-8 w-12 h-12 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20 transition-colors z-10"
          >
...
// Rating Component
const Rating = ({ rating, reviews = 0, showCount = true }) => {
  const numericRating = typeof rating === 'number' ? rating : parseFloat(rating) || 0;
  const numericReviews = typeof reviews === 'number' ? reviews : parseInt(reviews) || 0;
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-amber-400 font-bold">{numericRating.toFixed(1)}</span>
      {showCount && numericReviews > 0 && (
        <>
          <span className="text-gray-600">•</span>
          <span className="text-gray-400">{numericReviews} reviews</span>
        </>
      )}
    </div>
  );
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center">
        <div className="text-center">
          <div className="relative w-32 h-32 mx-auto mb-8">
            ...
            <Crown className="absolute inset-0 m-auto w-12 h-12 text-amber-400 animate-pulse" />
          </div>
          <p className="text-amber-400 text-xl animate-pulse">PREPARING YOUR ROYAL DISH...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <h2 className="text-3xl font-bold text-white mb-2">Dish Not Found</h2>
          <p className="text-gray-400 mb-8">The royal dish you're looking for isn't on our menu</p>
          <button
            onClick={() => navigate(-1)}
            className="px-8 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-bold hover:shadow-2xl hover:scale-105 transition-all"
          >
            Return to Menu
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-gray-900/80 backdrop-blur-xl border-b border-white/10">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-20">
            <button
              onClick={() => navigate(-1)}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 transition-all group"
            >
            ...
            <div className="flex items-center gap-3">
              <IconButton icon={Heart} onClick={() => setIsWishlisted(!isWishlisted)}>
                {isWishlisted && <span className="text-rose-400"></span>}
              </IconButton>
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="relative">
              <div className="absolute -inset-4 bg-gradient-to-r from-amber-500 to-rose-500 rounded-[2rem] opacity-20 blur-2xl animate-pulse" />
              <div className="relative rounded-[2rem] overflow-hidden">
                ...
                <div className="absolute top-6 left-6 flex gap-3">
                  <Badge variant="success">Signature Dish</Badge>
                  {dish.spiceLevel && (
                    <Badge variant="warning">{dish.spiceLevel} Spice</Badge>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <GradientText className="text-sm font-semibold tracking-wider">
                    ROYAL SIGNATURE
                  </GradientText>
                </div>
                
                <h1 className="text-5xl lg:text-6xl font-bold mb-4">
                  <GradientText>{dish.title}</GradientText>
                </h1>
                
                <p className="text-xl text-gray-300 mb-4">{dish.subtitle}</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <GlassCard className="p-4 text-center">
                  ...
                  <div className="font-semibold text-sm">{dish.preparationTime}</div>
                </GlassCard>
                
                <GlassCard className="p-4 text-center">
                  <Users className="w-6 h-6 text-amber-400 mx-auto mb-2" />
                  <div className="text-xs text-gray-400">Serves</div>
                  <div className="font-semibold text-sm">{dish.serves}</div>
                </GlassCard>
                
                <GlassCard className="p-4 text-center">
                  <ChefHat className="w-6 h-6 text-amber-400 mx-auto mb-2" />
                  <div className="text-xs text-gray-400">Chef</div>
                  <div className="font-semibold text-sm">{dish.chef}</div>
                </GlassCard>
                
                <GlassCard className="p-4 text-center">
                  <Award className="w-6 h-6 text-amber-400 mx-auto mb-2" />
                  <div className="text-xs text-gray-400">Experience</div>
                  <div className="font-semibold text-sm">{dish.chefExperience}</div>
                </GlassCard>
              </div>
              ...
              <GlassCard className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <div className="text-3xl font-bold">
                      <GradientText>{dish.price}</GradientText>
                    </div>
                  ...
                  <div className="flex items-center gap-4">
                    <button
                      onClick={() => setQuantity(Math.max(1, quantity - 1))}
                      className="w-10 h-10 rounded-full bg-white/5 border border-white/10 text-amber-400 text-xl hover:bg-white/10 transition-all"
                    >
                      −
                    </button>
                    <span className="text-2xl font-bold text-amber-400">{quantity}</span>
                    <button
                      onClick={() => setQuantity(quantity + 1)}
                      className="w-10 h-10 rounded-full bg-white/5 border border-white/10 text-amber-400 text-xl hover:bg-white/10 transition-all"
                    >
                ...
                <div className="flex gap-4">
                  <button className="flex-1 py-4 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-bold hover:shadow-2xl hover:scale-105 transition-all">
                    Book Now · ₹{totalPrice.toLocaleString('en-IN')}
                  </button>
                  <button className="px-6 py-4 bg-white/5 border border-white/10 text-amber-400 rounded-xl font-bold hover:bg-white/10 transition-all">
                    Add to Cart
                  </button>
                </div>
              </GlassCard>
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          {/* Elegant Tabs */}
          <div className="flex flex-wrap gap-2 mb-12 p-1 bg-white/5 rounded-2xl border border-white/10">
            {[
              { id: 'overview', label: 'Overview', icon: Info },
              { id: 'courses', label: 'Menu', icon: Utensils },
              { id: 'ingredients', label: 'Ingredients', icon: ShoppingBag },
              { id: 'pairing', label: 'Wine Pairing', icon: Wine },
              { id: 'reviews', label: 'Reviews', icon: MessageCircle }
            ].map((tab) => {
              ...
              {activeTab === 'overview' && (
                <div className="space-y-8">
                  <GlassCard className="p-8">
                    ...
                    <p className="text-gray-300 text-lg leading-relaxed mb-4">{dish.description}</p>
                    {dish.longDescription && (
                      <p className="text-gray-400 leading-relaxed">{dish.longDescription}</p>
                    )}
                  </GlassCard>
                  ...
                  {dish.highlights && dish.highlights.length > 0 && (
                    <GlassCard className="p-8">
                      <h3 className="text-2xl font-bold mb-6 flex items-center gap-3">
                        ...
                        <GradientText>Royal Highlights</GradientText>
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {dish.highlights.map((highlight, index) => (
                          <div key={index} className="flex items-center gap-3 p-4 bg-white/5 rounded-xl border border-white/5">
                    ...
                    <GlassCard key={index} className="p-8">
                      <div className="flex items-center gap-4 mb-4">
                        <div className="w-12 h-12 bg-gradient-to-br from-amber-500/20 to-orange-500/20 rounded-xl flex items-center justify-center">
                          {index === 0 && <Sunrise className="w-6 h-6 text-amber-400" />}
                          {index === 1 && <Sun className="w-6 h-6 text-amber-400" />}
                          {index === 2 && <Sunset className="w-6 h-6 text-amber-400" />}
                          {index === 3 && <Moon className="w-6 h-6 text-amber-400" />}
                        </div>
                        <div>
                  ...
                  {dish.ingredients.map((ingredient, index) => (
                    <GlassCard key={index} className="p-6 hover:scale-105 transition-transform">
                      <div className="flex items-start gap-4">
                        <div className="text-4xl bg-white/5 p-3 rounded-xl">{ingredient.icon}</div>
                        <div>
                  ...
                  {dish.winePairing.map((wine, index) => (
                    <GlassCard key={index} className="p-6">
                      <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <div className="flex-1">
                          <h4 className="text-xl font-bold text-white mb-1">{wine.wine}</h4>
                          <p className="text-amber-400 text-sm mb-2">{wine.region}</p>
                          <p className="text-gray-300 text-sm mb-3">{wine.pairing}</p>
                          <div className="text-lg font-bold text-amber-400">{wine.price}</div>
                        </div>
                      </div>
                    </GlassCard>
                  ))}
              ...
              <div className="sticky top-24 space-y-6">
                ...
                <GlassCard className="p-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                      ...
                      <span className="text-amber-400 font-semibold">{dish.availability}</span>
                    </div>
                    {dish.bestTime && (
                      <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                        <span className="text-gray-400">Best Time</span>
                        <span className="text-emerald-400 font-semibold">{dish.bestTime}</span>
                      </div>
                    )}
                    {dish.cuisineType && (
                      <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                        <span className="text-gray-400">Cuisine</span>
                        <span className="text-blue-400 font-semibold">{dish.cuisineType}</span>
                      </div>
                    )}
                  </div>
                ...
                <GlassCard className="p-6 bg-gradient-to-br from-amber-500/10 to-orange-500/10 border-amber-500/20">
                  ...
                  <p className="text-gray-300 italic text-sm mb-4">
                    "Every dish is prepared with the same love and care as I would for my own family. 
                    Each ingredient is hand-picked to ensure the highest quality."
                  </p>
                  <div className="flex items-center gap-3">
                    <img 
                      src="https://images.unsplash.com/photo-1566554273541-37a9ca77b91f?ixlib=rb-4.0.3&auto=format&fit=crop&w=200&q=80"
                    ...
                    />
                    <div>
                      <p className="font-bold text-white">{dish.chef}</p>
                ...
                <GlassCard className="p-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <button className="w-full flex items-center gap-3 p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-all">
                      ...
                      <span className="text-sm text-gray-300">royal@tavisha.com</span>
                    </button>
                    <button className="w-full flex items-center gap-3 p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-all">
                      <MapPin className="w-4 h-4 text-amber-400" />
                      <span className="text-sm text-gray-300">123 Royal Street, Mumbai</span>
                    </button>
                  </div>
                </GlassCard>
              </div>
          ...
          {dish.gallery && dish.gallery.length > 0 && (
            <section className="mt-16">
              <h3 className="text-2xl font-bold mb-8 flex items-center gap-3">
                ...
                <GradientText>Visual Journey</GradientText>
              </h3>
              <ImageGallery images={dish.gallery} title={dish.title} />
            </section>
          )}
