...
const GuestMemories = ({ selectedMood, onImageClick }) => {
  ...
  // Auto slide only if items exist
  useEffect(() => {
  ...
  return (
    <section className="py-16 bg-royal-maroon/10 rounded-3xl p-8">
      ...
      <div className="relative h-96 max-w-4xl mx-auto">
        ...
        {currentMemory && (
          <AnimatePresence mode="wait">
            <motion.div
              ...
              exit={{ opacity: 0, x: -100 }}
              transition={{ duration: 0.5 }}
              onClick={() => onImageClick(currentMemory)}
            >
              <img
        ...
        {/* Fallback when no memories found */}
        {filteredMemories.length === 0 && (
          <p className="text-center text-royal-maroon text-xl">
