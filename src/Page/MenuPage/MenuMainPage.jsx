...
// // src/components/GrandMenu.jsx
// import RoyalDishDetail from './Components/RoyalDishDetail'
// import React, { useState, useEffect, useRef } from 'react';
...
//   {
//     id: 'event-3',
//     title: 'DESSERT DEGUSTATION',
//     date: 'March 29, 2024',
//     time: '4:00 PM',
//     description: 'A journey through royal Indian desserts with Chef Priya Sharma.',
//     image: 'https://images.unsplash.com/photo-1551024506-0bccd828d307?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80',
//     price: 6000,
//     spots: 12,
...
// // Chef Detail Modal/Page
// const ChefDetailPage = ({ chefId, onBack }) => {
//   const chef = MASTER_CHEFS.find(c => c.id === chefId);
...
//         <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_30%_30%,rgba(212,175,55,0.1)_0%,transparent_50%)]" />
//       </div>

//       {/* Back Button */}
//       <button
//         onClick={onBack}
//         className="fixed top-24 left-8 z-50 group flex items-center gap-2 bg-black/50 backdrop-blur-xl text-yellow-500 px-6 py-3 rounded-full border border-yellow-500/30 hover:bg-yellow-500 hover:text-gray-900 transition-all duration-300"
//       >
...
//               ))}
//             </div>
//             <h1 className="text-7xl font-bold text-yellow-500 mb-4 font-serif">{chef.name}</h1>
//             <p className="text-2xl text-gray-300">{chef.title}</p>
//           </div>
//         </div>
//       </div>
...
//         {/* Tabs */}
//         <div className="flex gap-8 mb-12 border-b border-yellow-500/20">
//           {['story', 'signature', 'gallery', 'experience'].map((tab) => (
//             <button
//               key={tab}
...
//                 <h2 className="text-3xl font-bold text-yellow-500 mb-6">The Journey</h2>
//                 <p className="text-gray-300 text-lg leading-relaxed">{chef.bio}</p>
//                 <p className="text-gray-400 text-lg leading-relaxed italic">"{chef.philosophy}"</p>
                
//                 <div className="grid grid-cols-2 gap-6 mt-8">
//                   <div className="bg-gray-900/50 p-6 rounded-2xl border border-yellow-500/20">
//                     <h3 className="text-yellow-500 font-semibold mb-2">Years of Excellence</h3>
//                     <p className="text-3xl font-bold text-white">{chef.experience}</p>
//                   </div>
//                   <div className="bg-gray-900/50 p-6 rounded-2xl border border-yellow-500/20">
//                     <h3 className="text-yellow-500 font-semibold mb-2">Signature Dish</h3>
//                     <p className="text-xl font-bold text-white">{chef.signatureDish}</p>
//                   </div>
//                 </div>
//               </div>
              
//               <div className="space-y-6">
//                 <h2 className="text-3xl font-bold text-yellow-500 mb-6">Awards & Recognition</h2>
//                 {chef.awards.map((award, i) => (
//                   <div key={i} className="flex items-center gap-4 bg-gray-900/30 p-6 rounded-2xl border border-yellow-500/20">
//                     <Medal className="w-12 h-12 text-yellow-500" />
//                     <div>
//                       <p className="text-xl text-white">{award}</p>
//                     </div>
//                   </div>
//                 ))}
//               </div>
...
//                 <div key={i} className="group relative overflow-hidden rounded-3xl border border-yellow-500/20 hover:border-yellow-500/40 transition-all duration-500 hover:-translate-y-2">
//                   <img src={dish.image} alt={dish.name} className="w-full h-64 object-cover group-hover:scale-110 transition-transform duration-700" />
//                   <div className="absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-900/50 to-transparent" />
//                   <div className="absolute bottom-0 left-0 right-0 p-6">
//                     <h3 className="text-xl font-bold text-yellow-400 mb-2">{dish.name}</h3>
...
//               {chef.dishes.map((dish, i) => (
//                 <div key={i} className="relative h-80 rounded-3xl overflow-hidden">
//                   <img src={dish.image} alt={dish.name} className="w-full h-full object-cover" />
//                   <div className="absolute inset-0 bg-gradient-to-t from-gray-900 to-transparent" />
//                   <p className="absolute bottom-4 left-4 text-white text-xl font-bold">{dish.name}</p>
//                 </div>
//               ))}
//             </div>
//           )}
...
//               <h2 className="text-4xl font-bold text-yellow-500 mb-4">Book an Experience</h2>
//               <p className="text-xl text-gray-400 mb-8">Join Chef {chef.name} for an exclusive culinary journey</p>
//               <button className="bg-gradient-to-r from-yellow-500 to-yellow-600 text-gray-900 font-bold py-4 px-12 rounded-full text-xl hover:shadow-2xl hover:shadow-yellow-500/30 transition-all duration-300">
...
// // In the GrandMenu component, add this function to handle dish selection from the dish list
// const handleDishSelect = (dishId) => {
//   const dish = ROYAL_DISHES.find(d => d.id === dishId);
...
//     section.title.toLowerCase().includes(sectionSearchTerm.toLowerCase()) ||
...
//             isInWishlist={wishlist.includes(selectedDish.id)}
//           />
//         ) : !selectedSection ? (
//           <>
//             {/* CULINARY COLLECTIONS with Search */}
//             <section ref={sectionsRef} className="mb-20">
...
//                 </span>
//               </h2>
//               <p className="text-center text-gray-400 mb-8 text-lg max-w-3xl mx-auto">
...
//             {/* Section Header */}
//             <div className="mb-8 animate-slideUp">
//               <h2 className="text-5xl md:text-6xl font-bold text-yellow-500 mb-2 font-serif">
//                 {MENU_SECTIONS.find(s => s.id === selectedSection)?.title}
//               </h2>
...
//               <GrandSearchBar onSearch={setSearchTerm} />
//             </div>
...
// Dish Card Component
const GrandDishCard = ({ dish, onToggleWishlist, isInWishlist, onClick }) => {
  const [isHovered, setIsHovered] = useState(false);

  return (
    <div
      className="group relative bg-gray-900/50 backdrop-blur-sm rounded-2xl overflow-hidden border border-yellow-500/20 hover:border-yellow-500/40 transition-all duration-500 hover:-translate-y-2 cursor-pointer"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      onClick={onClick}
    >
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          {dish.isSignature && (
            <div className="bg-gradient-to-r from-yellow-500 to-yellow-600 text-gray-900 px-2 py-1 rounded-full text-xs font-bold flex items-center gap-1">
              <Crown className="w-3 h-3" />
              SIGNATURE
            </div>
          )}
          {dish.spiceLevel > 2 && (
            <div className="bg-red-500/80 text-white px-2 py-1 rounded-full text-xs font-bold flex items-center gap-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredSections.map((section, index) => (
                  <GrandSectionCard
                    ...
                    onSelect={(sectionId) => navigate(`/menu/${sectionId}`)}
                  />
                ))}
              </div>
            </section>
